...
class build {
	...
	function setup() {
		...
		echo "
			<h1>SETUP AIM</h1>
			<div>SERVER=localhost,HOST=$get->host</div>
			<form method='post' enctype='multipart/form-data'>
				Geef database gebruiker<br>
				<input type='text' name='username' value='aim' required><br>
				Geef database wachtwoord<br>
				<input type='password' name='password' required value='A!mUser'><br>
				<input type=submit value='Import' name='submit'>
			</form>";

		if($_POST[submit]){
			$fdest=__DIR__."/../config.json";
			echo "<ol><li>Create/modify $fdest</li>";
			$config=json_decode(file_get_contents(file_exists($fdest=__DIR__."/../config.json")?$fdest:__DIR__."/../setup/template.config.json"));
			$config->dbs->user=$_POST[username];
			$config->dbs->password=$_POST[password];
			$config->dbs->database='aim';
			$config->dbs->server='localhost';
			//die($fdest.stripslashes(json_encode($config,JSON_PRETTY_PRINT)));
			file_put_contents($fdest,stripslashes(json_encode($config,JSON_PRETTY_PRINT)));


			echo "<li>Connect to MS-SQL server</li>";
			$conn = sqlsrv_connect ( 'localhost', [Database=>'master',UID=>$_POST[username],PWD=>$_POST[password]]);
			echo "<li>Create AIM Database (if not exists)</li>";
			sqlsrv_query($conn,"IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = N'AIM') CREATE DATABASE [AIM]");
			//sqlsrv_query($conn,"select 1 as a");
			...
			echo "<li>Create AIM tables and sql (if not exists)</li>";
			require_once (__DIR__."/connect.php");
      ...
      //return;
        ...
        ENABLE TRIGGER ALL ON DATABASE
			...
			echo "<li>Create $fdest (if not exists)</li>";
			if (!file_exists($fdest=$_SERVER[DOCUMENT_ROOT]."/Web.config"))copy(__DIR__."/../setup/template.Web.config",$fdest);

			if (file_exists($fdest=$_SERVER[DOCUMENT_ROOT]."/sites/$get->host/api/v1/sql/$get->host.create.sql")){
				//$row=fetch_object(query("SELECT * FROM sys.databases WHERE name = N'dms'"));
				//if(!$row){
					echo "<li>Create $get->host database (if not exists)</li>";
					$sql=file_get_contents($fdest);
          // die($sql);
					querysql($sql);
				//}
				//else {
				//  echo"Database $get->host exists<br>";
				//}
			}

			if (file_exists($fdest=$_SERVER[DOCUMENT_ROOT]."/sites/$get->host/app/v1/config.json")){
				echo "<li>Create/modify $fdest</li>";
...
if (isset($_GET[import])) build::import();
if (isset($_GET[setup])) build::setup();
...
if (isset($_GET[getAccount])){
	...
	if (isset($_GET[download])) header("Content-disposition: attachment; filename=aliconnectAccount$row->accountId$row->accountName".date("Ymd_hi").".json");
	die(json_encode($row));
}
...
if (isset($_GET[sql])){
	...
	file_put_contents(__DIR__."/sql/aim.create.sql",$sql="SET NOCOUNT ON\r\nGO\r\nUSE $dbname\r\nGO".$sqlschema.$sql);
...
header('Content-type: application/json');
if(isset($_GET[download])) header("Content-disposition: attachment; filename=aliconnect_export_".date("Ymd_hi")."$filename.json");
