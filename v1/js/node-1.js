...
(function (){
  ...
  [process.argv[1], __dirname].forEach(path => {
  ...
  .on('connect', err => {
    console.log(config.client.system.id);
    if (config.data) {
      // console.debug(process.cwd(), config.client.system.id);
      res = require(process.cwd()+'/'+config.data);
